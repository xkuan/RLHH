...
from masterproblem import _MasterProblemBase
# from restricted_master_heuristics import _DivingHeuristic
...
class _MasterSolvePulp(_MasterProblemBase):
    ...
    def solve(self, relax, time_limit, final_step=False):
    ...
    def get_total_cost_and_routes(self, relax: bool):
        ...
        logger.debug("total cost = %s" % total_cost)
        if not total_cost:
            total_cost = 0
    ...
    def _solve(self, relax: bool, time_limit: Optional[int]):
        ...
        if self.final_step:
            ...
            if relax:   # 最后一步的松弛问题,需要重新设置变量
                # Set route variables to Continue
