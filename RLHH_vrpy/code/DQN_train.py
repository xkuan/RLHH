...
def get_config(args=None):
    ...
    # parser.add_argument('--solve_SP_method', default='gurobi', help="{gurobi, ESPPRC, SPPRC}")
    # parser.add_argument('--gurobi_msg', type=bool, default=False, help="gurobi OutputFlag")

    # parser.add_argument('--data_path', type=str, default=r'../c101.txt', help="")
    # parser.add_argument('--customerNum', type=int, default=10, help="")
    # parser.add_argument('--capacity', type=int, default=None, help="")
    # parser.add_argument('--sub_time_limit', type=int, default=600, help="")
    ...
    parser.add_argument('--run_mode', type=str, default="debug", choices=["debug", "test", "train"])
    ...
    parser.add_argument('--n_min', type=int, default=25, help="min customers of training instance")
    parser.add_argument('--n_max', type=int, default=35, help="max customers of training instance")
...
for i_episode in range(start_episode, num_episodes):
    ...
    state = env.reset(_instance_type=instance_type, run_mode=run_mode)
    for t in count():
        ...
        if run_mode == 'debug':
            print('ep:{}, t:{}, reward:{:.4f}, done:{}, objval:{:.4f}, heuristic:{}'.format(
                i_episode+1, t+1, reward, done, info['objval'], info['pricing_strategy'],
                ))
        # else:
        #     if t == 20:
        #         writer.add_scalar('iter_20_rewards', ep_reward, i_episode)
        #         writer.add_scalar('iter_20_objval', info['objval'], i_episode)
        #     if t == 40:
        #         writer.add_scalar('iter_40_rewards', ep_reward, i_episode)
        #         writer.add_scalar('iter_40_objval', info['objval'], i_episode)
        #     if t == 60:
        #         writer.add_scalar('iter_60_rewards', ep_reward, i_episode)
        #         writer.add_scalar('iter_60_objval', info['objval'], i_episode)
        ...
        if done:
            scheduler.step()
            # env.post_process()
            print("Episode {:3d}/{}, type: {}, instance: {}, n: {:2d}, iter: {:3d}, RMP_time: {:.2f}, SP_time: {:.2f}, "
                  "reward: {:.6f}, loss: {:.6f}, objVal: {:.4f}, gap: {:.4f}".format(
                i_episode + 1, num_episodes, instance_type, env.instance_name, env.num_customer,
                t + 1, sum(env.RMP_times), sum(env.SP_times), ep_reward, ep_loss, env.best_value, info["gap"]))
            with open (os.path.join(save_dir, f'running_logs/{test_case}.txt'), 'a') as file:
                file.write("Episode {:3d}/{}, type: {}, instance: {}, n: {:2d}, iter: {:3d}, RMP_time: {:.2f}, SP_time: {:.2f}, "
                           "reward: {:.6f}, loss: {:.6f}, objVal: {:.4f}, gap: {:.4f}\n".format(
                    i_episode + 1, num_episodes, instance_type, env.instance_name, env.num_customer,
                    t + 1, sum(env.RMP_times), sum(env.SP_times), ep_reward, ep_loss, env.best_value, info["gap"]))
            if run_mode in ['train', 'test']:
                ...
                writer.add_scalar('4_objval', env.best_value, i_episode)
                writer.add_scalar('5_episode_losses', ep_loss, i_episode)
                writer.add_scalar('6_iters', t+1, i_episode)
            break
