...
from subproblem_lp import _SubProblemLP
...
from subproblem_greedy import _SubProblemGreedy
from preprocessing import get_num_stops_upper_bound
from clarke_wright import _ClarkeWright, _RoundTrip
from checks import (
    check_arguments,
    check_consistency,
    check_feasibility,
...
class VehicleRoutingProblem:
    ...
    def initialize(
        ...
        self.load_capacity = data.max_load
        # xukuan, 10.13, 18:11
    ...
    def _pre_solve(self):
        ...
        if self.load_capacity and not self.pickup_delivery:
            num_stops = get_num_stops_upper_bound(
                self.G, self._max_capacity, self.num_stops, self.distribution_collection
            )
            self.num_stops = num_stops
            # logger.info("new upper bound : max num stops = %s" % num_stops)
        # Set empty route cost if use_all_vehicles = True
        if self.use_all_vehicles:
            n_vehicle_types = len(self.G["Source"]["Sink"]["cost"])
            self.G["Source"]["Sink"]["cost"] = n_vehicle_types * [1e10]
    ...
    def _attempt_solve_best_edges(self, vehicle=None, duals=None, route=None):
        more_columns = False
        consider, method = map_edge_heuristics[pricing_strategy]
        # _pricing_strategy = map_edge_heuristics[consider, method]
        ...
        # If pickup and delivery, initial routes are Source-pickup-delivery-Sink
            ...
            for v in self.G.nodes():
                if "request" in self.G.nodes[v]:
                    obvious_route = ["Source", v, self.G.nodes[v]["request"], "Sink"]
                    edges = [
                        ("Source", v),
                        (v, self.G.nodes[v]["request"]),
                        (self.G.nodes[v]["request"], "Sink"),
                    ]
                    if self.time_windows:
                        check_pickup_delivery_time_windows(self.G, edges)
                    self._initial_routes.append(obvious_route)
