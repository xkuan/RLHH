...
def get_config(args=None):
    ...
    parser.add_argument('--run_mode', type=str, default="test", choices=["debug", "test", "train"])
    ...
    parser.add_argument('--num_episodes', type=int, default=200, help="number of training episode")
...
def get_net(_net_type):
    _policy_net = QNetTwinDuel(mid_dim=64, state_dim=num_featrues, action_dim=n_actions).to(device)
    _target_net = QNetTwinDuel(mid_dim=64, state_dim=num_featrues, action_dim=n_actions).to(device)
    return _policy_net, _target_net

def select_action(_state):
    EPS_END = config.epsilon
    EPS_START = 0.9
    EPS_DECAY = 200     # (0.9 - 0.05) * exp(-1.* 1000 / 200) = 0.0057
    global steps_done
        ...
        if done:
            ...
            with open (os.path.join(save_dir, f'running_logs/{test_case}.txt'), 'a') as file:
                file.write("Episode {:3d}/{}, type: {}, instance: {}, n: {:2d}, iter: {:3d}, RMP_time: {:.2f}, SP_time: {:.2f}, "
                    ...
                    i_episode + 1, num_episodes, instance_type, env.instance_name, env.num_customer,
                    t + 1, sum(env.RMP_times), sum(env.SP_times), ep_reward, ep_loss, env.best_value, info["gap"]))
            if run_mode in ['train', 'test'] and i_episode > 0:
